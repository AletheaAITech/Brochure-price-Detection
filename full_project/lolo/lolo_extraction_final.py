...
# pdff_to_jpg('/home/webtunix/Music/price_detection/lolo/images/page4.jpg')
...
def extract_data(files):
    ...
    for i in files:
        ...
        print('extracting data from Page '+str(count))
        dff=get_roii(image)
        df_list.append(dff)
        count+=1
    final_df = pd.concat(df_list, ignore_index=True)
    final_df.columns = ['Products', 'Brand', 'New_price', 'Previous_price']
    final_df.to_csv(BASE_DIR+'/csvs/lolo_extracted2.csv',index=False)
...
if __name__ == "__main__":
    pdff_to_jpg('Lolo_250222_150322.pdf')
    time.sleep(2)
    files=glob.glob(BASE_DIR+'/images/*.jpg')
    files = sorted(files, key=lambda x: float(re.findall("(\d+)", x)[0]))
    extract_data(files)
